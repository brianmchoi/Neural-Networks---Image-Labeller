...
\newcommand{\dueDate}{Monday, Oct 26, 2020 11:59 PM}
...
{\Large \question Example Feed Forward and Backpropagation}
...
\part In the following questions you will derive the matrix and vector forms of the previous equations which define out neural network. These are what you should hope to program in order to keep your program under the Gradescope time-out.\\
...
$$\boldsymbol{\beta}=
    ...
    \begin{subparts}
        \subpart[1] By examining the shapes of the initial weight matrices, how many neurons do we have in the first hidden layer of the neural network? (Not including the bias neuron)
        ...
        \subpart[1] What is the vector $\av$ whose elements are made up of the entries $a_j$ in Equation \ref{eqn:a}. Write your answer in terms of $\alpha$ and $\xv^{(1)}$.
...
\clearpage
\part We will now derive the matrix and vector forms for the backpropagation algorithm.
...
\begin{subparts}
    \subpart[3] The derivative of the softmax function with respect to $b_k$ is as follows:
    ...
    Using this, write the derivative $\frac{\partial \ell}{\partial b_k}$ in a smart way such that you do not need this indicator function. Write your solutions in terms of $\hat{y_k},y_k$. Show your work below.
    ...
    \subpart[1] Explain in one short sentence why must we go back to using the matrix $\betav^*$ (The matrix $\betav$ without the first column of ones) when calculating the matrix $\frac{\partial \ell}{\partial \alpha}$?
    ...
    \subpart[1] What is the derivative $\frac{\partial \ell}{\partial \zv}$? Your answer should be in terms of $\frac{\partial \ell}{\partial \bv}$ and $\betav^*$.
...
\begin{subparts}
    ...
    \subpart[1] What is $a_2$?
    
    \begin{your_solution}[title=$a_2$,height=2cm,width=3cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=Work,height=4cm,width=12cm]
    % YOUR ANSWER 
    ...
    \subpart[1] What is $z_3$?
    
    \begin{your_solution}[title=$z_3$,height=2cm,width=3cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=Work,height=4cm,width=12cm]
    % YOUR ANSWER 
    ...
    \subpart[1] What is $b_1$?
    
    \begin{your_solution}[title=$b_1$,height=2cm,width=3cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=Work,height=4cm,width=12cm]
    % YOUR ANSWER 
...
\part Now use the results of the previous question to run backpropagation over the network and update the weights. Use learning rate $\eta=1$. 
...
\begin{subparts}
    ...
    \subpart[2] What is the updated value of the weight ${\alpha}_{3,4}$?
...
\clearpage
{\Large \question Empirical Questions}
\label{sec:interp}
...
Please submit computer-generated plots for (ai) and (bi). Note: we expect it to take about \textbf{5 minutes} to train each of these networks.
...
\clearpage
\subpart[2] Examine and comment on the the plots of training and validation cross-entropy. What is the effect of changing the number of hidden units?
...
\subpart[2] Examine and comment on the the plots of training and validation cross-entropy. How does adjusting the learning rate affect the convergence of cross-entropy of each dataset?
...
\end{your_solution}\end{questions}
...
\begin{itemize}
    ...
    \item You may assume that the input data will always have the same output label space (i.e. $\{0,1,\ldots,9\}$). Other than this, do not hard-code any aspect of the datasets into your code. We will autograde your programs on multiple (hidden) data sets that include different examples.
...
\begin{enumerate}
    ...
    \item How do we get $\adj{z_j}$? You guessed it: apply the chain rule yet again. This time, however, there are \emph{multiple} children of $z_j$ in the computation graph; they are $b_1, b_2, \ldots b_K$. Applying the chain rule gives us that $\adj{z_j} = \sum_{k=1}^K \adj{b_k}\frac{\partial  b_k}{\partial  z_j} = \sum_{k=1}^K \adj{b_k}\beta_{kj}$. Substituting back into the equation above gives:
